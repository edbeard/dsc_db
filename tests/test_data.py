...
class TestData(unittest.TestCase):
    ...
    def test_add_distributor_info(self):
        ...
        expected = {'voc': {'OpenCircuitVoltage': {'raw_units': '(mV)', 'raw_value': '756', 'specifier': 'Voc', 'units': '(10^-3.0) * Volt^(1.0)', 'value': [756.0]}},
                    'dye': {'Dye': [{'raw_value': 'XY1', 'specifier': 'dye',
                                     'smiles': {'context':'dict','value':'CCCCC(CC)COc1ccc(c(OCC(CC)CCCC)c1)c2ccc(cc2)N(c3ccc(cc3)c4ccc(OCC(CC)CCCC)cc4OCC(CC)CCCC)c5ccc(cc5)c6ccc(c7sc8c9sc(cc9C(CC(CC)CCCC)(CC(CC)CCCC)c8c7)c%10ccc(cc%10)\\C=C(C#N)\\C(O)=O)c%11nsnc6%11'},
